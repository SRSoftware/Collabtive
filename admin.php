...
elseif ($action == "adduser") {
    ...
    if (!empty($_FILES['userfile']['name'])) {
        ...
        if ($user->edit($id, $cleanPost["name"], "" , $cleanPost["email"], $cleanPost["tel1"], $cleanPost["tel2"], $cleanPost["company"], $cleanPost["zip"], $cleanPost["gender"], $cleanPost["web"], $cleanPost["address1"], $cleanPost["address2"], $cleanPost["state"], $cleanPost["country"], $tags, $cleanPost["locale"], $avatar, $cleanPost["rate"], $cleanPost["openid"])) {
        ...
        if ($user->edit($id, $cleanPost["name"], "", $cleanPost["email"], $cleanPost["tel1"], $cleanPost["tel2"], $cleanPost["company"], $cleanPost["zip"], $cleanPost["gender"], $cleanPost["web"], $cleanPost["address1"], $cleanPost["address2"], $cleanPost["state"], $cleanPost["country"], $tags, $cleanPost["locale"], "", $cleanPost["rate"], $cleanPost["openid"])) {
            if (!empty($cleanPost["newpass"]) and !empty($cleanPost["repeatpass"])) {
                $user->admin_editpass($id, $cleanPost["newpass"], $cleanPost["repeatpass"]);
            }
            header("Location: admin.php?action=users&mode=edited");
